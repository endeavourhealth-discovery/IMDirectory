...
describe("EclService ___ axios success", () => {
  ...
  it("can get ECLSearch", async () => {
    ...
    const eclSearchRequest = ECLQueryRequestSchema.parse({ query: { is: { iri: "testString" } }, includeLegacy: false, limit: 1000 });
  ...
  it("can getEcl", async () => {
    const testQuery = QuerySchema.parse({ is: { iri: "testEntity" } });
