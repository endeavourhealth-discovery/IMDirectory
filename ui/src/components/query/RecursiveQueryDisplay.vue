<template>
  <div class="feature" v-for="(match, index) of matches">
    <div>
      ...
      <span v-if="match.path && isArrayHasLength(match.path)">
        <span v-if="match.path[0].match && isObjectHasKeys(match.path[0].match, ['where']) && isArrayHasLength(match.path[0].match.where)">
