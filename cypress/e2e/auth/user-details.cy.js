describe("User details", () => {
  ...
  it("starts with user data", () => {
    cy.findByTestId("user-details-avatar").should("have.attr", "src").should("include", "001-man");
    cy.findByTestId("user-details-username").should("have.value", Cypress.env("CYPRESS_LOGIN_USERNAME"));
    cy.findByTestId("user-details-firstname").should("have.value", "Cypress");
    cy.findByTestId("user-details-lastname").should("have.value", "Test");
    cy.findByTestId("user-details-email").should("have.value", "test@example.com");
  });
