...
} from "@/helpers/modules/CopyConceptToClipboard";
...
describe("CopyConceptToClipboard", () => {
    ...
    it("can copy concept to clipboard", async () => {
      const concept = {
        ...
        "http://www.w3.org/2000/01/rdf-schema#label":
          "Scoliosis caused by radiation (disorder)",
      };
  ...
  it("can convert conceptObjectToCopyString ___ arrayString", () => {
    expect(
      conceptObjectToCopyString(
        "subtypes",
        ["Acquired scoliosis (disorder)", "Acrodysplasia scoliosis (disorder)"],
        0,
        1,
        CONFIG
      )
  ...
  it("can convert conceptObjectToCopyString ___ number", () => {
    expect(
      conceptObjectToCopyString("subtypes", 9, 0, 1, CONFIG)
    ).toStrictEqual({
      label: "Has sub types",
