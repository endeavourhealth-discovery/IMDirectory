...
step("Landing page is displayed", async () => {
  await pw.page.waitForSelector("#landing-content", { timeout: 60000 });
...
step("Suggested section exists", async () => {
  ...
  await suggested.waitFor({ state: "visible", timeout: 60000 });
});

step("Quick links section exists", async () => {
  const quickLinks = pw.page.locator("h2").filter({ hasText: "Quick links" });
  await quickLinks.waitFor({ state: "visible", timeout: 60000 });
});
...
step("Favourites section exists", async () => {
  ...
  await favourites.waitFor({ state: "visible", timeout: 60000 });
});
