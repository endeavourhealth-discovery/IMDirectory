<template>
  <div class="string-single-select-container">
    ...
    <InputNumber v-model="userInput" class="p-inputtext-lg input-text" data-testid="text-input" />
...
const emit = defineEmits<{ updateClicked: [payload: number] }>();
...
onMounted(() => {
  if (props.value) userInput.value = props.value as number;
});
watch(
  () => props.value,
  newValue => {
...
function updateEntity(data: number) {
...
function updateValueVariableMap(data: number) {
