...
import { HiddenUnchangedLinesInfo } from "./utils/get-segment";
...
<template>
  <tr class="expand-line">
    <td :colspan="4" :class="{ 'has-lines-before': segment.hasLinesBefore, 'has-lines-after': segment.hasLinesAfter }">
      <slot name="expand-line" v-if="$slots.expandLine" :has-line-before="segment.hasLinesBefore" :has-line-after="segment.hasLinesAfter" />
      <template v-else>
        <button v-if="segment.hasLinesBefore" @click="() => onExpandBefore(index)(expandMoreLinesLimit)">⭡ Show {{ expandMoreLinesLimit }} lines before</button>
        <button @click="() => onExpandAll(index)()">⭥ Show all unchanged lines</button>
        <button v-if="segment.hasLinesAfter" @click="() => onExpandAfter(index)(expandMoreLinesLimit)">⭣ Show {{ expandMoreLinesLimit }} lines after</button>
      </template>
    </td>
  </tr>
</template>
