  ...
  <div class="editor-dialog-container">
    <Splitter class="query-splitter">
      <SplitterPanel :size="30" :minSize="10" style="overflow: auto" class="splitter-left">
        <QueryNavTree @removeProperty="deleteProperty" :base-entity-match="baseEntityMatch" :editMatches="editMatches" />
      </SplitterPanel>
      <SplitterPanel :size="70" :minSize="10" style="overflow: auto" class="splitter-right">
        <div v-for="(editMatch, index) in editMatches" :key="editMatch['key']">
          <Divider v-if="index" align="center">
            <div :class="editBoolMatch" @click="toggleBoolMatch">{{ editBoolMatch }}</div>
          </Divider>
          <EditMatch @onDeleteProperty="deleteProperty" :base-entity-match="baseEntityMatch" :edit-match="editMatch" />
        </div>
      </SplitterPanel>
    </Splitter>
    <div class="footer">
...
function deleteProperty(property: any) {
  const removeIndex = editMatches.value.findIndex(editMatch => (editMatch as any).key === property["key"]);
